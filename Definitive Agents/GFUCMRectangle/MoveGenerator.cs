            ...
            for (int i = k + 1; i < platformList.Count; i++)
            {
                ...
                else
                {
                    if (p2.yTop == p.yTop)
                    {
                        ...
                        {
                            trajectoryAdder.AddAdjacent(ref platformList, ref p, p2.rightEdge - p2.leftEdge < 8 ? 50 : 10 * (p2.rightEdge - p2.leftEdge), p.rightEdge, RectangleShape.Shape.HORIZONTAL, p2);
                            trajectoryAdder.AddAdjacent(ref platformList, ref p2, p2.rightEdge - p2.leftEdge < 8 ? -50 : -10 * (p2.rightEdge - p2.leftEdge), p2.leftEdge, RectangleShape.Shape.HORIZONTAL, p);
                        }
                        else if (p.leftEdge == p2.rightEdge + 1)
                        {
                            trajectoryAdder.AddAdjacent(ref platformList, ref p, p2.rightEdge - p2.leftEdge < 8 ? -50 : -10 * (p2.rightEdge - p2.leftEdge), p.leftEdge, RectangleShape.Shape.HORIZONTAL, p2);
                            trajectoryAdder.AddAdjacent(ref platformList, ref p2, p2.rightEdge - p2.leftEdge < 8 ? 50 : 10 * (p2.rightEdge - p2.leftEdge), p2.rightEdge, RectangleShape.Shape.HORIZONTAL, p);
                        }
                    }
                }
            }
