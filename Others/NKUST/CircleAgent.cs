...
namespace GeometryFriendsAgents
{
    ...
    public class CircleAgent : AbstractCircleAgent
    {
        ...
        private string agentName = "circle";
        ...
        private RectangleRepresentation rectangleInfoOld;
        ...
        public override void Setup(CountInformation nI, RectangleRepresentation rI, CircleRepresentation cI, ObstacleRepresentation[] oI, ObstacleRepresentation[] rPI, ObstacleRepresentation[] cPI, CollectibleRepresentation[] colI, Rectangle area, double timeLimit)
        {
            ...
            levelArray.getRectPlant(rectangleInfo);
        ...
        public override void SensorsUpdated(int nC, RectangleRepresentation rI, CircleRepresentation cI, CollectibleRepresentation[] colI)
        {
            ...
            rectangleInfo = rI;
            circleInfo = cI;
            levelArray.removeRectPlant(rectangleInfoOld);
            levelArray.getRectPlant(rectangleInfo);
            rectangleInfoOld = rectangleInfo;
            collectiblesInfo = colI;
            /*if (collectiblesInfo != colI)//&&rectangleInfo.X==rectangleInfoOld.X&& rectangleInfo.Y == rectangleInfoOld.Y && rectangleInfo.Height == rectangleInfoOld.Height)
            {
                collectiblesInfo = colI;
                levelArray.CreateLevelArray(collectiblesInfo, obstaclesInfo, rectanglePlatformsInfo);
                levelArray.getRectPlant(rectangleInfo);
            ...
            if (Math.Abs(circleInfo.VelocityX)<10&& Math.Abs(circleInfo.VelocityY) < 10 && !nextEdge.HasValue&& Math.Abs(rectangleInfo.VelocityX) < 6 && Math.Abs(rectangleInfo.VelocityY) < 6)
            {
                platform.SetUp(levelArray.GetLevelArray(),levelArray.initialCollectiblesInfo.Length);
            }
        }
