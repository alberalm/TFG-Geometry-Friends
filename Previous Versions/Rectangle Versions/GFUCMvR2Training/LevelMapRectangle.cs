        ...
        protected override void GenerateMoveInformation()
        {
            ...
            for (int k = 0; k < platformList.Count; k++)
            {
                ...
                if (p.real)
                {
                    ...
                    Parallel.For(0, platformList.Count, i =>
                    {
                        RectangleShape.Shape s = RectangleShape.Shape.VERTICAL; //Shape not critical
                        // Right moves
                        int x = p.rightEdge + 1;
                        while (x < GameInfo.LEVEL_MAP_WIDTH && levelMap[x, p.yTop] != PixelType.OBSTACLE && levelMap[x, p.yTop] != PixelType.PLATFORM)
                        {
                        ...
                        if (levelMap[x, p.yTop] == PixelType.PLATFORM &&
                            ...
                            x - p.rightEdge < 16)
                        {
                            AddTrajectory(ref p, 1, MoveType.BIGHOLEDROP, (x + p.rightEdge) / 2 + 1, s, new Platform(-1));
                        }
                        // Left moves
                        x = p.leftEdge - 1;
                        while (x >= 0 && levelMap[x, p.yTop] != PixelType.OBSTACLE && levelMap[x, p.yTop] != PixelType.PLATFORM)
                        ...
                        if (levelMap[x, p.yTop] == PixelType.PLATFORM &&
                            ...
                            p.leftEdge - x < 16)
                        {
                            AddTrajectory(ref p, -1, MoveType.BIGHOLEDROP, (x + p.leftEdge) / 2 + 1, s, new Platform(-1));
                        }
                    });
                }
            }
